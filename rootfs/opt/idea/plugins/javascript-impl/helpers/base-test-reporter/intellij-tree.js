...
Node.prototype.getStartMessage = function () {
  ...
  if (this.state === NodeState.REGISTERED) {
    ...
    return '##teamcity[' + commandName + ' nodeId=\'' + escapeAttributeValue(this.id) + '\' running=\'true\']';
  }
  throw Error("Unexpected node state: " + this.state);
};
...
TestNode.prototype.addStdOut = function (out) {
  if (isString(out)) {
    this.tree.sendMessage('testStdOut', {nodeId: this.id, out: out});
  }
};
...
TestNode.prototype.addStdErr = function (err) {
  if (isString(err)) {
    this.tree.sendMessage('testStdErr', {nodeId: this.id, out: err});
  }
};
